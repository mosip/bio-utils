...
public class NistDataQualityAnalyser {
	...
	public static void main(String[] args) {
		if (args != null && args.length >= 1) {
			...
			LOGGER.info("main :: serverIPAddress :: Argument {} ", serverIPAddress);
			...
			LOGGER.info("main :: serverPort :: Argument {} ", serverPort);
			...
			LOGGER.info("main :: serverPath :: Argument {} ", serverPath);
			...
			LOGGER.info("main :: contentType :: Argument {} ", contentType);
			...
			LOGGER.info("main :: contentCharset :: Argument {} ", contentCharset);
			...
			LOGGER.info("main :: results :: Argument {} ", results);
		...
		try {
			...
			for (File file : fileList) {
				...
				try {
					...
					LOGGER.info(" Decoded Time >>> Time Taken : {} in milliseconds",
							(System.currentTimeMillis() - startTimeNist));
					if (nistRecord != null) {
						// Finger Records
						ProcessFingerprintRecords threadFingerprint = new NistDataQualityAnalyser().new ProcessFingerprintRecords(
								"fingerprint", biometricFolderPath, personId, nistRecord, contentType, contentCharset,
								serverIPAddress, serverPort, serverPath, results);
						threadFingerprint.start();

						// Iris Records
						ProcessIrisRecords threadIris = new NistDataQualityAnalyser().new ProcessIrisRecords("iris",
								biometricFolderPath, personId, nistRecord, contentType, contentCharset, serverIPAddress,
								serverPort, serverPath, results);
						threadIris.start();

						// Face Records
						ProcessFaceRecords threadFace = new NistDataQualityAnalyser().new ProcessFaceRecords("face",
								biometricFolderPath, personId, nistRecord, contentType, contentCharset, serverIPAddress,
								serverPort, serverPath, results);
						threadFace.start();
					}
				...
				LOGGER.info("For file {} Total Time Taken : {} in milliseconds", file.getAbsolutePath(),
						(System.currentTimeMillis() - startTimeOneFile));
				LOGGER.info("                                                                       ");
				LOGGER.info(" **********************************RESULT Ended**********************************");
			}
			long endTime = System.currentTimeMillis();
			LOGGER.info("Total Time Taken : {} in milliseconds", (endTime - startTime));
	...
	public abstract class ProcessBiometricRecords implements Runnable {
		...
		public ProcessBiometricRecords(String modality, String biometricFolderPath, String personId,
				BiometricInformationExchange nistRecord, String contentType, String contentCharset,
				String serverIPAddress, String serverPort, String serverPath, String jsonResults) {
		...
		public synchronized JSONObject getQualityInfoWithJson(String id, String type, String data) {
			...
			Request request = new Request.Builder()
					.url("http://" + getServerIPAddress() + "" + getServerPort() + "" + getServerPath()).post(body)
					.build();
			...
			if (response != null && response.isSuccessful()) {
				...
				try {
					...
					LOGGER.error("getQualityInfoWithJson eror", e);
				}
			}
			return null;
		}

		public synchronized void writeToCsvFile(CsvSchema csvSchema, JsonNode jsonNode) throws IOException {
			synchronized (NistDataQualityAnalyser.class) {
				...
				if (!Objects.isNull(outputFile) && !outputFile.exists() && outputFile.createNewFile()) {
		...
		public synchronized CsvSchema getCsvSchema() {
			...
			case "face":
				builder = CsvSchema.builder().addColumn("file").addColumn("background_deviation")
						...
						.addColumn("opencv_profile_face_found").addColumn("over_exposure")
						.addColumn("over_exposure_face").addColumn("quality").addColumn("sap_code")
						.addColumn("skin_ratio_face").addColumn("skin_ratio_full").addColumn("image_area")
						.addColumn("image_channels").addColumn("image_height").addColumn("image_ratio")
						.addColumn("image_width").addColumn("openbr_left_eye_x").addColumn("openbr_left_eye_y")
						.addColumn("openbr_right_eye_x").addColumn("openbr_right_eye_y")
						.addColumn("opencv_face_center_of_mass_x").addColumn("opencv_face_center_of_mass_y")
						.addColumn("opencv_face_offset_x").addColumn("opencv_face_offset_y").addColumn("opencv_face_x")
						.addColumn("opencv_face_y").addColumn("opencv_left_eye_x").addColumn("opencv_left_eye_y")
						.addColumn("opencv_mouth_x").addColumn("opencv_mouth_y").addColumn("opencv_nose_x")
						.addColumn("opencv_nose_y").addColumn("opencv_right_eye_x").addColumn("opencv_right_eye_y");
				break;
			case "fingerprint":
				builder = CsvSchema.builder().addColumn("file").addColumn("Width").addColumn("Height")
						.addColumn("NFIQ2").addColumn("Quantized").addColumn("Resampled").addColumn("UniformImage")
			...
			return !Objects.isNull(builder) ? builder.build().withoutHeader() : null;
		}
		...
		public synchronized JsonNode toJsonNode(JSONObject jsonObject)
	...
	public class ProcessFingerprintRecords extends ProcessBiometricRecords {
		public ProcessFingerprintRecords(String modality, String biometricFolderPath, String personId,
				BiometricInformationExchange nistRecord, String contentType, String contentCharset,
				String serverIPAddress, String serverPort, String serverPath, String jsonResults) {
			super(modality, biometricFolderPath, personId, nistRecord, contentType, contentCharset, serverIPAddress,
					serverPort, serverPath, jsonResults);
		}
		...
		public void run() {
			for (PackageHighResolutionGrayscaleImageRecord fingerRecord : getNistRecord()
				...
				while (result == null) {
					result = getQualityInfoWithJson(
							getModality() + "#SEP#" + getPersonId() + "#SEP#" + biometricSubType, "wsq", imageBase64);
				}
	...
	public class ProcessIrisRecords extends ProcessBiometricRecords {
		...
		public void run() {
			for (PackageCBEFFBiometricDataRecord irisRecord : getNistRecord().getIrisImageRecordList()) {
				String biometricSubType = getBiometricSubTypeName(
						irisRecord.getIrisImage().getIrisEyePosCode().getValue());
				String imageBase64 = irisRecord.getIrisImage().getBinaryBase64();

				JSONObject result = null;
				while (result == null) {
					result = getQualityInfoWithJson(
							getModality() + "#SEP#" + getPersonId() + "#SEP#" + biometricSubType, "jpeg", imageBase64);
				}
